...
void dummy_reference(int N, int *matA, int *matB, int *output)
{
...
//            input_file >> matA[i * N + j];
    ...
    // int *matB = (int *)aligned_alloc(128, sizeof(int) * N * N);
    for(int i = 0; i < N; ++i)
        for(int j = 0; j < N; ++j)
		matB[i*N+j] = cnt++;
//            input_file >> matB[i * N + j];
    ...
    begin = TIME_NOW;
    // __itt_task_begin(domain, __itt_null, __itt_null, handle_cust);
